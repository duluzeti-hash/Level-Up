...
// Read allowed origins from environment variable (comma-separated). Default to the previous values.
const allowedOriginsEnv = process.env.ALLOWED_ORIGINS || 'https://level-up.fly.dev,http://localhost:8080';
  ...
  socket.on('checkOrder', ({ orderedTips }) => {
    ...
    if (everyoneFinished) {
      ...
      const roundResult = {
        ...
        playersScreenshot: rankedPlayers.map(p => ({ id: p.id, name: p.name, score: p.score }))
      };
